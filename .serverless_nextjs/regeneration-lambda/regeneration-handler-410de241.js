...
const s3StorePage = async (options) => {
    const { S3Client } = await Promise.resolve().then(function () { return require('./S3Client-59a48fa9.js'); });
    ...
    const { PutObjectCommand } = await Promise.resolve().then(function () { return require('./PutObjectCommand-be1e7fdf.js'); });
    await Promise.all([
...
const renderPageToHtml = async (page, req, res, renderMode) => {
    ...
    try {
        ...
        console.log("Exception occurred, falling back to using page's rendering function for html");
