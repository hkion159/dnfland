...
![Screenshot 2021-11-09 at 00 18 53](https://user-images.githubusercontent.com/59219773/140783721-36bf2c76-c3cf-4750-ab7e-549a98350d3b.jpg)
...
종합게시판에서 캐릭터 세팅에 대해 질문하고 도움을 얻거나 답변할 수도 있습니다.
...
- **문제 6: popper를 이용한 popover의 위치와 화살표가 어긋남**
  ...
  - 시도 2: 일반 자바스크립트에선 display 스타일을 통해 popover를 보이고 감춥니다. 리액트만의 jsx 문법 특징인 jsx문 내에서의 조건문을 활용하여 popover를 보일지 결정하게 했습니다.
...
- **문제 8: redux state가 다른 페이지로 이동할 때마다 초기화 됨**
  ...
  - 시도 2: 계속되는 검색 중에 next-redux-wrapper가 페이지 이동 시마다 클라이언트에서 생성하는 스토어를 서버의 스토어와 합친다는 것을 알게 되었습니다. 그것이 HYDRATE라는 액션이고 리듀서의 상태는 클라이언트의 상태이며 액션의 payload가 서버의 상태임을 알았습니다. 그래서 HYDRATE 액션이 디스패치되면 서버가 아니라 클라이언트의 상태를 리턴하도록 했습니다.
...
- **문제 9: getServerSideProps 함수 내에서 받은 리덕스 상태가 브라우저와 달리 항상 초기값임**
  ...
  - 시도 2: 긴 시간 검색 끝에 상태는 클라이언트와 서버에 각각 따로 존재한다는 것을 알았습니다. 상태 변경 디스패치는 클라이언트의 상태만 변경시켰고 서버사이드렌더링 때의 상태는 서버의 상태를 받아왔습니다. 클라이언트와 서버의 상태를 합치는 방법을 검색했습니다.
    - 성공: next-redux-wrapper의 공식 깃허브 문서에서 브라우저의 쿠키를 이용하는 next-redux-cookie-wrapper 모듈을 이용하면 클라이언트와 서버에 공유된 정보를 이용할 수 있다는 것을 알았습니다. 그 모듈의 깃허브를 정독하여 사용법을 익히고 프로젝트에 적용했습니다.


- **문제 10: 앱을 Vercel에 배포하고 실행했는데 속도가 개발 서버보다 훨씬 느려짐**

  - 시도 1: 코드 전체를 최적화가 안 된 부분이 있는지 검토했습니다.
    - 실패 원인: 속도 문제는 코드가 아니었습니다. 실제 웹사이트의 속도는 아주 많은 요소들이 결정한다는 것을 나중에 알았습니다.

  - 시도 2: 검색 결과 같은 문제를 겪었던 사람의 글을 발견했습니다. Vercel 서버가 서울에도 있는 것으로 알고 있었지만 무료 요금제를 쓰면 기본 서버 위치를 워싱턴에서 변경할 수 없다는 것을 알았습니다. 그래서 api 요청이 매우 느려지는 결과를 만들었던 것이었습니다.
    - 결과: aws lambda에 다시 앱을 배포했지만 도메인 이전은 60일이 경과되어야 가능해서 하지 못했습니다.


## 느낀 점

짧게 10개만 서술했지만 그 외에도 개발 과정 중에 크고 작은 문제들을 정말 많이 마주쳤습니다.

그럴 때마다 문제의 원인을 파악하고 해결하면서 공부가 되었고 프레임워크를 조금 더 잘 이해할 수 있었습니다.

그리고 한참 해결 방안을 찾을 수 없던 문제를 모듈 버전을 낮춘 것만으로 해결하는 등 문제의 원인은 여러가지가 있을 수 있다는 것을 배웠습니다.

개발이라는 일이 코드 작성이 전부가 아니고 에러나 문제를 해결하는 데 매우 많은 시간을 쓰는 일이라는 것을 직접 경험했습니다.

그 결과 스스로 만들고 해결하는 과정을 거치면서 어떤 기술과 환경에도 금방 적응하는 능력을 길렀습니다.

또한 jsx문이 길어지면서 알아보기 힘들어질 때 코드를 작은 단위로 분리하고 클린하게 작성하는 것이 중요하다는 것을 느꼈습니다.

이 개인 프로젝트를 진행하면서 기술적인 측면과 개발이라는 직무의 측면에서 많은 것을 배울 수 있었습니다.
